...
const NAV_SECTIONS = [
  {
    title: "核心功能",
    items: [
      { href: "/agent", label: "工作台", icon: LayoutDashboard },
      { href: "/agent/stations", label: "站长管理", icon: Users },
      { href: "/agent/complaints", label: "代处理工单", icon: TicketCheck },
    ],
  },
  {
    title: "业务管理",
    items: [
      { href: "/agent/region", label: "区域管理", icon: Map },
      { href: "/agent/settlement", label: "收入结算", icon: Wallet },
    ...
    <div className={cn("flex h-full flex-col", className)}>
      ...
      <div className="flex h-16 items-center gap-3 px-5 border-b border-white/5">
        ...
        <div className="flex flex-col">
          ...
          <span className="text-xs text-slate-400">代理管理后台</span>
        </div>
      </div>
      ...
      <div className="border-t border-white/5 p-4">
        <div className="flex items-center gap-3 px-1">
          <div className="flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-br from-blue-500 to-blue-600">
            <span className="text-xs text-white font-medium">代</span>
          </div>
